...
u8 	idata DATA_FLAG=0;	//数据有效标志


unsigned char UartGroup;			//user define  ; 串口组号

u8 	idata GetRequest;	//读取请求标志
	   ...
	   if(RI)
		{
			...
			if(COM1.B_RX_OK == 0)
			{
				if(DATA_FLAG) //数据有效
			   {
			   		...
			   		//if( COM1.RX_Cnt >= 5)
					if( (RX1_Buffer[COM1.RX_Cnt++] = SBUF) == 0xF7)	
					{
						COM1.RX_Cnt = 2; 
						DATA_FLAG=0; 
						RX1_Buffer[0]=0;
						if(RX1_Buffer[2]==(0xa0|myaddr) && RX1_Buffer[3]==0xF7)   
						{
							GetRequest = 1;		//读取请求标志位置位
						   	/*
						   	TX1_write2buff(0xF0);
							TX1_write2buff(AVA_LiquidLevel);
							TX1_write2buff(myaddr);//
							TX1_write2buff(0);//
							TX1_write2buff(FirstData);  //液面状态 0：未检测到页面 1；检测到液面
							TX1_write2buff(0xF7);
						*/
			   ...
			   {
			   	 	RX1_Buffer[1]=SBUF;
				   	if(RX1_Buffer[0]==0xF0 && RX1_Buffer[1]==AVA_Compass)
					{
					  	COM1.RX_Cnt = 2;
						DATA_FLAG = 1;
					}
					RX1_Buffer[0]=RX1_Buffer[1];
			   }		
			}
		}
